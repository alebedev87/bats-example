...
# REPO_USERNAME and REPO_PASSWORD are expected to be set externally
...
if [ -n "${VALUES}" ]; then
    if [ "${EXPAND_CRED_VALUES}" = "true" ]; then
        VALUES=$(envsubst '$REPO_USERNAME$REPO_PASSWORD' <<<"${VALUES}")
    fi
    OPTS+=(--set "${VALUES}")
fi
...
if [ -n "${REPOSITORY}" ]; then
    ...
    if [ "${PROVIDE_REPO_CRED}" = "true" ]; then
        OPTS+=(--username "${REPO_USERNAME}" --password "${REPO_PASSWORD}")
    fi
fi
